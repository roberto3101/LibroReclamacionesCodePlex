...
// Registrar en historial
	...
	if err != nil {
		...
		go func(codigo, nombre, tipo, mensaje string) {
			if err := enviarEmailMensajeCliente(codigo, nombre, tipo, mensaje); err != nil {
				log.Printf("❌ Error enviando email de mensaje: %v", err)
			} else {
				log.Printf("✅ Email de mensaje enviado correctamente para %s", codigo)
			}
		}(codigoReclamoFull, nombreCliente, tipoSolicitud, req.Mensaje)
	}
