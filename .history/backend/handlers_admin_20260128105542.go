...
func listarReclamosAdminHandler(c *gin.Context) {
	...
	for rows.Next() {
		var r struct {
            ...
            DiasRestantes *int
            NombreAdminAtendio sql.NullString // <--- VARIABLE NUEVA
        }
        // AGREGAMOS &r.NombreAdminAtendio AL FINAL DEL SCAN
        if err := rows.Scan(&r.ID, &r.CodigoReclamo, &r.TipoSolicitud, &r.Estado, &r.NombreCompleto,
            ...
            reclamos = append(reclamos, gin.H{
                ...
                "nombre_admin_atendio":   nullToInterface(r.NombreAdminAtendio), // <--- CAMPO NUEVO EN JSON
            })
        }
	}
