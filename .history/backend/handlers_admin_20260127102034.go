// =============================================================================
// AGREGAR ESTAS RUTAS AL FINAL DE TU main.go EXISTENTE
// Justo antes de la línea: srv := &http.Server{...
// =============================================================================
...
var jwtSecret = getEnv("JWT_SECRET", "tu-secret-key-super-segura-cambiala-en-produccion")
...
func cambiarEstadoReclamoHandler(c *gin.Context) {
	...
	pool.Exec(ctx, `
		...
		VALUES ($1, 'CAMBIO_ESTADO', 'RECLAMO', $2, $3, $4)
	`, userID, id, fmt.Sprintf(`{"estado_anterior":"%s","estado_nuevo":"%s"}`, estadoAnterior, req.Estado), c.ClientIP())
...
func responderReclamoHandler(c *gin.Context) {
	...
	pool.Exec(ctx, `
		...
		VALUES ($1, 'RESPONDER', 'RECLAMO', $2, $3)
...
// =============================================================================
// AGREGAR ESTAS RUTAS EN TU router.Group("/api")
// =============================================================================
...
/*
	...
	// Rutas protegidas de admin (requieren JWT)
	admin := api.Group("/admin")
	admin.Use(authMiddleware())
	{
		// Auth
		admin.POST("/auth/logout", func(c *gin.Context) {
			c.JSON(http.StatusOK, gin.H{"success": true, "message": "Sesión cerrada"})
		})
		admin.GET("/auth/me", func(c *gin.Context) {
			c.JSON(http.StatusOK, gin.H{
				"success": true,
				"data": gin.H{
		...
		// Reclamos (ADMIN y SOPORTE)
		admin.GET("/reclamos", listarReclamosAdminHandler)
		admin.GET("/reclamos/:id", obtenerReclamoHandler) // Reutilizar handler existente
		...
		adminOnly.Use(rolAdminMiddleware())
		{
