...
func enviarEmails(reclamo ReclamoCreado, req CrearReclamoRequest, tipoBien string) error {
	...
	if req.Departamento != nil && req.Distrito != nil {
		ubicacion = fmt.Sprintf("%s / %s - %s", *req.Departamento, *req.Provincia, *req.Distrito)
		if req.Domicilio != nil && *req.Domicilio != "" {
			ubicacion = *req.Domicilio + " (" + ubicacion + ")"
		}
	}

	// 2. Enviar email al ADMINISTRADOR (Con teléfono y ubicación)
	m := gomail.NewMessage()
	...
	// Pasamos req.Telefono y la variable ubicacion
	...
	// 3. Enviar email al CLIENTE (Se mantiene igual, simple)
	if req.AceptaCopia {
...
<div style="background: #eef2ff; padding: 10px; border-radius: 5px; margin-bottom: 15px;">
	<div style="font-weight: bold; margin-bottom: 5px; border-bottom: 1px solid #cad4f0;">Datos de Contacto:</div>
	<div><strong>Nombre:</strong> %s</div>
	<div><strong>Email:</strong> %s</div>
	<div><strong>Teléfono:</strong> <a href="tel:%s" style="color: #1e40af; font-weight: bold;">%s</a></div>
	<div><strong>Ubicación:</strong> %s</div>
</div>
...
<div class="field"><div class="label">🖊️ Firma Digital:</div><div><a href="%s/api/reclamos/%s/firma" target="_blank" style="color: #1e40af; text-decoration: underline;">Ver firma del consumidor</a></div></div>
		...
		tipo, codigo, fechaLimite, tipo, nombre, email, telefono, telefono, ubicacion, tipoBienBg, tipoBienColor, tipoBien, descripcion, montoHTML, detalle, config.BackendURL, codigo)
}
