...
func enviarEmailMensajeCliente(codigoReclamo, nombreCliente, tipoSolicitud, mensaje string) error {
	m := gomail.NewMessage()
	m.SetHeader("From", config.SMTPFrom)
	m.SetHeader("To", config.EmailSoporte)
...
<p style="margin: 8px 0 0 0; color: #c4b5fd; font-size: 15px;">Reclamo: <strong style="color: #ffffff;">%s</strong></p>
</td>
</tr>
...
// Registrar en historial
	...
	// Enviar email al administrador (asíncrono)
	go func() {
		if err := enviarEmailMensajeCliente(codigoReclamoFull, nombreCliente, tipoSolicitud, req.Mensaje); err != nil {
			log.Printf("⚠️ Error enviando email de mensaje: %v", err)
		}
	}()
