...
interface Reclamo {
	...
	nombre_admin_atendio?: string | null;
}
...
export default function ReclamosList() {
    ...
    const [stats, setStats] = useState({ total_reclamos: 0, pendientes: 0, en_proceso: 0, resueltos: 0 });
    ...
    const fetchStats = async () => {
        try {
           ...
           const res = await fetch(`${API_URL}/api/admin/dashboard/stats`, { headers: { 'Authorization': `Bearer ${token}` } });
	...
	// Recargar data cuando cambie página o filtros
	useEffect(() => {
		// Debounce simple para búsqueda (opcional, o llamar directo)
		const timer = setTimeout(() => {
			fetchReclamos();
		}, 300);
		return () => clearTimeout(timer);
	}, [filtroEstado, searchTerm, page]);
	...
	const fetchReclamos = async () => {
		try {
			...
			// Construcción dinámica de URL con Paginación y Búsqueda
			const queryParams = new URLSearchParams({
				page: page.toString(),
				limit: '30', // Solicitamos 30 por página
			});

			if (filtroEstado) queryParams.append('estado', filtroEstado);
			if (searchTerm) queryParams.append('search', searchTerm); // Ahora el backend filtra

			const response = await fetch(`${API_URL}/api/admin/reclamos?${queryParams.toString()}`, {
				headers: {
			...
			<div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                        ...
                        <div className="text-2xl font-bold text-gray-900 mt-1">{stats.total_reclamos}</div>
                    </div>
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                        ...
                        <div className="text-2xl font-bold text-blue-600 mt-1">{stats.en_proceso}</div>
                    </div>
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                        <div className="text-sm text-gray-600">Resueltos</div>
                        <div className="text-2xl font-bold text-green-600 mt-1">{stats.resueltos}</div>
                    </div>
                </div>
									...
									<tbody className="bg-white divide-y divide-gray-200">
										{reclamos.map((reclamo) => (
											<tr key={reclamo.id} className="hover:bg-gray-50 transition">
												...
												<td className="px-6 py-4">
													<span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${reclamo.tipo_solicitud === 'RECLAMO'
												...
												<td className="px-6 py-4">
													<div className="flex flex-col items-start gap-1">
														...
														{reclamo.nombre_admin_atendio && (
															<span className="text-[10px] text-gray-500 font-medium italic">
																Por: {reclamo.nombre_admin_atendio}
															</span>
														)}
													</div>
												</td>
												<td className="px-6 py-4">
													...
													<div className={`text-xs ${reclamo.dias_restantes !== null && reclamo.dias_restantes < 3
				...
				{!loading && !error && reclamos.length > 0 && (
					<div className="flex flex-col items-center gap-4 border-t border-gray-200 pt-4">
						<div className="text-sm text-gray-600">
							Mostrando {reclamos.length} registros (Total: {pagination.total})
						</div>
